...
<!-- // TASK-098-fix -->
`export --verify`는 먼저 파일 형식, fact 스키마, 개수, facts+events 체크섬을
검사하고, 별도의 임시 홈에서 검증된 왕복 이식을 수행합니다. 왕복 단계에서는
supersedes 연결과 provenance를 포함한 모든 논리 fact 행, 원래 논리 순서대로
다시 작성된 events를 비교하고, 대표 recall 질의가 같은 fact ID를 같은 순서로
반환하는지 확인합니다.

이는 기억 이식성 검사입니다. 백업 무결성 증명이 아니며, 다른 복사본이나 저장
장소에서 데이터가 삭제되었음을 증명하지도 않습니다.
