# glymph v0 빌드 스펙 (정본 — 구조설계 v1의 실행형)
...
## 0. 공통
- `GLYMPH_HOME` env로 홈 오버라이드, 기본 `~/.glymph`. 모든 모듈은 홈 경로를 인자/env로만 받는다 (하드코딩 금지).
- 브랜드 문자열은 `src/brand.js`의 `export const BRAND = "glymph"` 한 곳만.
...
- `render.js renderViews(store, home)`: scope별 md — active fact를 subject 그룹으로 불릿. 프론트매터 `generated: true` + "glymph이 생성한 읽기전용 뷰" 1줄.
...
- 원칙(불변): 기본 켜짐 + `glymph telemetry off` 1커맨드 + 전송 스키마 공개 문서 + "전송 내역 보기" 커맨드. 끄면 0바이트 — 이 구멍은 버그가 아니라 신뢰 카피("끄면 진짜 아무것도 안 갑니다").
