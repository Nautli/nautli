...
- At the start of a conversation or when context is needed, check relevant memories with \`briefing\` or \`recall\` first. If the briefing includes a [nautli] status line (e.g. a stalled patrol), relay it to the user.
...
- 대화를 시작하거나 작업 맥락이 필요할 때 \`briefing\` 또는 \`recall\`로 관련 기억을 먼저 확인한다. briefing에 [nautli] 상태 줄(예: 순찰 멈춤)이 있으면 사용자에게 전달한다.
