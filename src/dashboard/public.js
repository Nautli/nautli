...
(function(){
  ...
  async function loadCheckup(){var previous=state.checkup;var previousJson=previous===null?null:JSON.stringify(previous);var previousError=state.checkupError;try{state.checkup=await api("/api/checkup/status");state.checkupError=null;}catch(error){state.checkupError="상태를 못 읽었어요. 잠시 후 다시 시도합니다";clearTimeout(state.checkupTimer);state.checkupTimer=setTimeout(function(){loadCheckup();},10000);if(state.tab==="setup"&&state.status){var failedSlot=document.getElementById("checkup-slot");if(failedSlot)failedSlot.innerHTML=checkupSlot();else setupView();}return;}
    ...
    if(state.checkupWatch&&previous&&(previous.state==="none"||previous.state==="dismissed")&&state.checkup.state==="running"){toast("진단이 시작됐어요. 이 화면에서 진행 상황이 보여요.");state.checkupWatch=null;var modal=document.getElementById("modal");if(modal&&!modal.classList.contains("hidden"))modal.classList.add("hidden");}
    if(state.checkup.state==="running"||(state.checkupWatch&&Date.now()-state.checkupWatch<30*60*1000&&(state.checkup.state==="none"||state.checkup.state==="dismissed"))){state.checkupTimer=setTimeout(function(){var before=state.checkup&&state.checkup.state;loadCheckup().then(function(){if(before==="running"&&state.checkup.state==="done")toast("건강검진이 끝났어요. 결과가 설정 탭에 있어요.");});},5000);}
  ...
  function checkupAgentPrompt(){return 'nautli 기억 건강검진을 돌려줘. 터미널에서 npx nautli checkup "'+(state.checkupPath||"")+'" 를 실행하면 진행 로그가 나와. 끝나면 요약 JSON의 중복·모순 건수를 알려주고, report_file 리포트를 읽고 핵심을 짚어줘.';}
