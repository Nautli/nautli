...
.nav button{width:100%;border:0;border-left:2px solid transparent;background:transparent;color:var(--muted-foreground);padding:9px 10px;border-radius:var(--radius-sm);text-align:left}
.nav button:hover{background:var(--accent);color:var(--foreground)}
.nav button.active{color:var(--foreground);background:var(--secondary);border-left-color:var(--ai-action-accent)}
...
@media(max-width:720px){
  ...
  .header{min-height:52px;padding:10px 14px;flex:1;flex-direction:row;align-items:center;justify-content:space-between}
  .brand{font-size:18px}
  .dawn{align-self:stretch;width:2px;height:auto}
  .nav{padding:6px 8px;flex-direction:row;overflow:auto}
  .nav button{width:auto;flex:0 0 auto;border-left:0;border-bottom:2px solid transparent;text-align:center}
  .nav button.active{border-left-color:transparent;border-bottom-color:var(--ai-action-accent)}
...
<div class="shell">
  ...
  <nav class="nav" aria-label="대시보드"><button data-tab="setup">설정</button><button data-tab="graph">그래프</button><button data-tab="review">카드 <span id="pending-badge" class="badge hidden">0</span></button><button data-tab="memory">기억</button></nav></aside>
...
(function(){
  ...
  if(!state.continuity)state.continuity={choice:"나는 커밋 메시지를 한국어로 쓴다",custom:"",a:"ready",b:"locked",c:"ready",factId:"",factClaim:"",since:"",crossSince:"",recalled:null};
  ...
  function continuityWaitCopy(){var elapsed=Math.max(0,Date.now()-Date.parse(state.continuity.since||new Date().toISOString()));if(elapsed<15000)return "붙여넣으셨나요?";if(elapsed<60000)return "Claude가 응답하면 자동으로 감지됩니다";return '감지가 안 되면 MCP 연결 문제일 수 있어요 <button class="btn quiet" data-copy="nautli doctor">진단 실행</button>';}
  ...
  function ensureContinuityPolling(){if(state.continuityPolling)return;var kind=state.continuity.a==="waiting"?"remember":state.continuity.c==="waiting"?"recall":null;if(!kind){stopContinuityPolling();return;}state.continuityPolling=true;void pollContinuity(kind);}
  async function pollContinuity(kind){var waiting=kind==="remember"?state.continuity.a==="waiting":state.continuity.c==="waiting";if(!waiting){stopContinuityPolling();return;}clearTimeout(state.continuityTimer);state.continuityTimer=null;var since=kind==="remember"?state.continuity.since:state.continuity.crossSince;try{var data=await api("/api/activity?since="+encodeURIComponent(since));waiting=kind==="remember"?state.continuity.a==="waiting":state.continuity.c==="waiting";if(!waiting){stopContinuityPolling();return;}var match=data.events.find(function(item){if(item.source!=="mcp")return false;if(kind==="remember")return item.type==="remember"&&item.claim===(state.continuity.factClaim||continuityClaim());return item.type==="recall"&&Array.isArray(item.hits)&&item.hits.includes(state.continuity.factId);});if(match){if(kind==="remember"){state.continuity.a="done";state.continuity.b="ready";state.continuity.factId=match.fact_id;state.continuity.factClaim=match.claim;state.continuity.factResult=match.result||"added";}else state.continuity.c="done";saveContinuity();stopContinuityPolling();if(state.tab==="setup")setupView();return;}}catch(error){}
    waiting=kind==="remember"?state.continuity.a==="waiting":state.continuity.c==="waiting";if(!waiting){stopContinuityPolling();return;}updateContinuityWait();state.continuityTimer=setTimeout(function(){state.continuityTimer=null;void pollContinuity(kind);},2000);
  }
  ...
  async function loadCheckup(){var previous=state.checkup;var previousJson=previous===null?null:JSON.stringify(previous);var previousError=state.checkupError;try{state.checkup=await api("/api/checkup/status");state.checkupError=null;}catch(error){state.checkupError="상태를 못 읽었어요. 잠시 후 다시 시도합니다";clearTimeout(state.checkupTimer);state.checkupTimer=setTimeout(function(){loadCheckup();},10000);if(state.tab==="setup"&&state.status){var failedSlot=document.getElementById("checkup-slot");if(failedSlot)failedSlot.innerHTML=checkupSlot();else setupView();}return;}
    ...
    if(state.checkup.state==="running"){state.checkupTimer=setTimeout(function(){var before=state.checkup&&state.checkup.state;loadCheckup().then(function(){if(before==="running"&&state.checkup.state==="done")toast("건강검진이 끝났어요. 결과가 설정 탭에 있어요.");});},5000);}
  ...
  async function openCheckupModal(){var found=(await api("/api/checkup/candidates")).candidates;state.checkupCandidates=found;state.checkupPath=found[0]?found[0].path:"";var body=found.map(function(cand,index){return '<label class="card step" style="cursor:pointer;margin-bottom:8px"><input type="radio" name="checkup-path" value="'+esc(cand.path)+'" '+(index===0?'checked':'')+'><div><h2>'+esc(cand.label)+' <span class="badge">'+(cand.kind==="obsidian"?"옵시디언":"Claude 하네스")+'</span></h2><p>노트 '+esc(cand.notes)+'개 · '+esc(cand.path)+'</p></div></label>';}).join("");body+='<div class="inline-field"><input class="field" id="checkup-custom" placeholder="'+(found.length?"또는 폴더 경로 직접 입력 (~/…)":"진단할 폴더 경로를 입력하세요 (~/…)")+'"><button class="btn" data-checkup-preflight>폴더 확인</button></div><div id="checkup-preflight"></div>';openModal("내 기억 건강검진","선택한 폴더의 노트 텍스트가 내 Claude 구독을 거쳐 Anthropic에서 처리됩니다. 요약·점수만 로컬에 저장되고 어디에도 업로드되지 않습니다.","",'<button class="btn primary" data-checkup-start disabled>진단 시작</button>',body);if(state.checkupPath)await loadCheckupPreflight(state.checkupPath,[]);else renderCheckupPreflight();}
  ...
  function continuityBlock(){
    var c=state.continuity;var examples=["나는 커밋 메시지를 한국어로 쓴다","내 기본 브랜치는 main이다","배포 전에는 항상 테스트를 먼저 돌린다"];
  ...
  function setupView(){var s=state.status.setup;var r=s.required;var done=[r.store,r.mcp,r.instructions,r.daemon].filter(function(item){return item.complete;}).length;var firstKey=!r.store.complete?"store":!r.mcp.complete?"mcp":!r.instructions.complete?"instructions":!r.daemon.complete?"daemon":null;function main(key,label,attrs){return '<button class="btn '+(firstKey===key?'primary':'')+'" '+attrs+'>'+label+'</button>';}
    var html=setupHero();if(s.complete){html+=checklistBlock();app.innerHTML=html;if(state.checklistOpen==="continuity")ensureContinuityPolling();return;}html+='<div class="progress-row"><span class="section-title">필수 연결 '+done+'/4</span><div class="progress"><i style="width:'+(done*25)+'%"></i></div></div>';var daemonWarn=r.daemon.plist_exists&&(!r.daemon.health.healthy||r.daemon.health.stale);var daemonDesc=r.daemon.complete?"마지막 실행 "+fmt(r.daemon.health.last_run)+" · 다음 실행 "+fmt(nextRunMs(r.daemon)):daemonWarn?(r.daemon.health.exists?"마지막 소화가 오래됐거나 실패했어요. 지금 소화 테스트로 확인해 주세요.":"설치됐어요. 지금 소화 테스트 한 번으로 연결을 확인해 주세요."):"자는 동안 중복을 합치고 모순을 카드로 만들어요. 매일 밤 3:30.";var digestButton='<button class="btn quiet" data-setup="digest" '+(state.digesting?'disabled':'')+'>'+(state.digesting?'소화 중… 최대 2분':'지금 소화 테스트')+'</button>';
  ...
  function reviewView(){
    ...
    if(state.cards.length===0){
      ...
      if(!setup.complete){app.innerHTML=html+'<div class="card empty">카드는 밤 소화가 만들어요. 먼저 설정을 완료해 주세요.<div class="actions"><button class="btn primary" data-tab="setup">설정으로 가기</button></div></div>';return;}
  ...
  function startGraph(graph){
    ...
    var colors={};function readGraphColors(){var styles=getComputedStyle(document.documentElement);colors={edge:styles.getPropertyValue("--graph-edge").trim(),node:styles.getPropertyValue("--graph-node").trim(),hub:styles.getPropertyValue("--graph-hub").trim(),superseded:styles.getPropertyValue("--graph-superseded").trim(),contradiction:styles.getPropertyValue("--graph-contradiction").trim(),duplicate:styles.getPropertyValue("--graph-duplicate").trim(),text:styles.getPropertyValue("--foreground").trim()};}readGraphColors();var themeMedia=typeof window.matchMedia==="function"?window.matchMedia("(prefers-color-scheme: dark)"):null;function handleThemeChange(){readGraphColors();draw();}if(themeMedia){if(typeof themeMedia.addEventListener==="function")themeMedia.addEventListener("change",handleThemeChange);else if(typeof themeMedia.addListener==="function")themeMedia.addListener(handleThemeChange);}
    ...
    function draw(){if(!canvas.isConnected)return;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,width,height);ctx.save();ctx.translate(width/2+camera.x,height/2+camera.y);ctx.scale(camera.zoom,camera.zoom);
      ...
      nodes.forEach(function(node){var radius=nodeRadius(node);ctx.beginPath();ctx.arc(node.x,node.y,radius,0,Math.PI*2);ctx.fillStyle=node.kind==="scope"?colors.hub:colors.node;if(node===hovered){ctx.shadowColor=colors.superseded;ctx.shadowBlur=14/camera.zoom;ctx.strokeStyle=colors.superseded;ctx.lineWidth=1.5/camera.zoom;ctx.stroke();}ctx.fill();ctx.shadowBlur=0;if(node.kind==="scope"){ctx.fillStyle=colors.text;ctx.font=(12/camera.zoom)+'px system-ui,"Apple SD Gothic Neo","Noto Sans KR","Segoe UI",sans-serif';ctx.fillText(node.label,node.x+(12/camera.zoom),node.y+(4/camera.zoom));}});ctx.restore();
    }
    ...
    var observer=typeof ResizeObserver==="function"?new ResizeObserver(resize):null;if(observer)observer.observe(stage);resize();if(!simLive){simLive=true;requestAnimationFrame(simulate);}state.graphCleanup=function(){running=false;if(observer)observer.disconnect();if(themeMedia){if(typeof themeMedia.removeEventListener==="function")themeMedia.removeEventListener("change",handleThemeChange);else if(typeof themeMedia.removeListener==="function")themeMedia.removeListener(handleThemeChange);}};
  }
  ...
  document.addEventListener("click",async function(event){
    ...
    var continuityCopy=event.target.closest("[data-continuity-copy]");if(continuityCopy){var claim=continuityClaim();if(!claim){toast("기억할 습관을 한 문장으로 적어 주세요.",true);return;}state.continuity.since=new Date(Date.now()-1000).toISOString();state.continuity.a="waiting";state.continuity.factClaim=claim;saveContinuity();await copyText("기억해줘: "+claim,"Claude Code에 붙여넣을 문장을 복사했어요.");setupView();ensureContinuityPolling();return;}
