...
// TASK-104: 리졸버 판정 이벤트의 policy_version 상수(§6 D2).
export const RESOLVER_POLICY_VERSION = "resolver@2";
    ...
    const updated = queue.map((entry) => {
      ...
      try {
        if (entry.type === "capture") {
          ...
          // TASK-104: 리졸버 판정 — reason은 resolver:<decision>, policy는 RESOLVER_POLICY_VERSION.
          const resolverMetaStamp = {
          ...
          if (decision === "a_wins" || decision === "b_wins") {
            ...
            if (loser.status === STATUS.ACTIVE) {
              ...
              }, "daemon", resolverMetaStamp);
            }
          } else if (decision === "both_invalid") {
            const today = new Date().toLocaleDateString("sv-SE");
            if (a.status === STATUS.ACTIVE) {
              store.transition(a.id, STATUS.INVALIDATED, { t_invalid: today }, "daemon", resolverMetaStamp);
            }
            if (b.status === STATUS.ACTIVE) {
              store.transition(b.id, STATUS.INVALIDATED, { t_invalid: today }, "daemon", resolverMetaStamp);
            }
          }
          stats.resolved += 1;
          changed = true;
