...
function runBatch(batch, config, cwd) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // 대화형 응답을 해버린다 (실측 사고: onebrain 리포 cwd에서 judge가 코드 리뷰를 시작함)
  ...
  for (let offset = 0; offset < pairs.length; offset += BATCH_SIZE) {
    ...
    try {
      ...
      if (result.parsedCount === 0 && batch.length > 0) {
        ...
        if (rawLog) fs.appendFileSync(rawLog, `--- ${new Date().toISOString()} batch@${offset} RETRY (0 parsed)\n${result.rawStdout}\n`);
        result = await runBatch(batch, config, sandbox);
      }
      if (rawLog) fs.appendFileSync(rawLog, `--- ${new Date().toISOString()} batch@${offset}\n${result.rawStdout}\n`);
      judgments.push(...result.judgments);
    } catch (error) {
      // 배치 하나의 실패(스폰 에러·타임아웃)가 밤 전체를 날리지 않게 격리 — 해당 쌍은 안전 no-op
      if (rawLog) fs.appendFileSync(rawLog, `--- ${new Date().toISOString()} batch@${offset} FAILED: ${error.message}\n`);
