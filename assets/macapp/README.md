...
`nautli-menubar.swift`는 NSStatusItem으로 상주하며 90초마다 `/api/status.pending`을 폴링해 대기 뱃지를 표시하고, 대시보드 열기·새로고침·끄기 메뉴를 제공한다. `installApp`이 네이티브 런처와 함께 빌드하고 `com.nautli.menubar` LaunchAgent(`KeepAlive`)로 상주시킨다.
