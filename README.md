...
## 빠른 시작
```bash
cd ~/Desktop/glymph && npm install   # 이미 됨
node src/cli.js init                    # ~/.glymph 생성
...
```bash
claude mcp add glymph -- node ~/Desktop/glymph/src/mcp/server.js
...
- **`glymph restore <id>`**: `.archive/`행 fact CLI 복구 동사 — 리뷰카드 오답 정정과 연결 (Hermes 계약)
