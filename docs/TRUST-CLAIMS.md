...
| A3 | 오병합 미관측(정성 서술) — 수치는 내부 게이트 전용 (README Measured quality) | 홀드아웃 3볼트 결과(`bug4city/onebrain-vault-holdout` results/, 허브 2026-07-13 기록). rule of three: 95% 상한 ~12% | **F3 해소(2026-07-24, 유저 승인)**: 7/16 합의대로 README에서 0/24 수치 철회 → "none observed so far + 표본 작아 공개벤치(사전등록) 출시 시 수치 공개" 정성 서술로 교체. 수치 재노출은 TASK-099 공개벤치 발행과 동시에만 | 자동병합 표본 누적 시 갱신(ivan duplicate 304건 재라벨이 최저비용 확대 경로) | 오병합 1건 발견 즉시 서술 철회+INCIDENT 발동 |
